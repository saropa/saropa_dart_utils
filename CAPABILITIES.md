...
> Generated by `tool/gen_capabilities.py` from the documented public API under `lib/`. Run it after adding utilities to keep this complete.
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `race` | method | Async More: race, allSettled, retry N times. |
| `allSettled` | method | Attempts all [futures] and returns results (values or error+stackTrace pairs). |
| `retryTimes` | method | Retries [fn] up to [times] attempts; rethrows on final failure. |
...
| `mapBatched` | method | Batch async (e.g. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Function` | constructor | Returns a wrapper that cancels previous in-flight calls when invoked again; the superseded future throws [CancelPreviousException]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Duration` | constructor | Default duration the circuit stays open before allowing a retry (half-open). |
...
| `debounce` | method | Debounce (time) — invokes [fn] after [delay] of no further calls. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `delay` | method | Delay by duration (Future.delayed wrapper). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Duration` | constructor | Default base delay for exponential backoff. |
| `exponentialBackoff` | method | Returns delay for attempt [attempt] (0-based): base * 2^attempt, capped at [maxDelay]. |
...
| `Function` | constructor | Callback invoked every [interval] while running. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `raceFirst` | method | Returns result of first future that completes successfully; others are not cancelled (fire-and-forget). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Duration` | constructor | Default delay between retry attempts. |
| `retryWithPolicy` | method | Retries [fn] up to [maxAttempts] with [delay] between attempts. |
| `retryWithJitter` | method | Exponential backoff with jitter: delay = base * 2^attempt + random(0, jitter). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Duration` | constructor | Default initial backoff delay. |
| `retryWithBackoff` | method | Retry with backoff (exponential or linear). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `mapSequential` | method | Sequential async map (one after another). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `debounceStream` | method | Returns a stream that re-emits values from [source], but suppresses any value that is followed by another within [duration]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `windowCount` | method | Buffers [stream] into lists of [count] elements. |
...
|--------|------|-------------|
| `VoidCallback` | typedef | Callback with no arguments. |
| `throttle` | method | Throttle (time) — max one call per interval. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `timeoutWithFallback` | method | Timeout with fallback value. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `withTimeout` | method | Runs [fn] with [timeout]; on timeout returns [fallback] or rethrows. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `gzipEncode` | method | Gzip-encodes [bytes] using `dart:io`. |
| `gzipDecode` | method | Gzip-decodes [bytes] using `dart:io`. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `gzipEncode` | method | Gzip-encodes the given bytes. |
| `gzipDecode` | method | Gzip-decodes the given bytes. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Function` | constructor | Memoize sync function (by argument equality). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `balancedPartitionIndices` | method | Greedy: assign each element of [values] to current smallest partition. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `firstFitBinPacking` | method | Assigns each [itemWeights] to a bin; [capacity] per bin. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `chunksWithOverlap` | method | Splits [list] into chunks of [chunkSize] with [overlap] between consecutive chunks. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `columnValues` | method | Returns a single column [key] from [rows] as List<Object?>. |
| `toColumnar` | method | Returns map of key -> list of values for all keys in [rows]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `damerauLevenshteinDistance` | method | Returns Damerau–Levenshtein distance (insert, delete, substitute, transpose). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `greedySetCover` | method | Returns subset of indices into [sets] that covers [universe]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `histogramFixed` | method | Build histogram: [values] binned by [edges] (sorted). |
| `histogramQuantile` | method | Bin boundaries at quantiles (e.g. |
...
| `maxNonOverlappingIntervals` | method | Returns a maximal set of non-overlapping intervals from [intervals], chosen by earliest end time (greedy). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `buildInvertedIndex` | method | Builds term -> list of document indices containing that term. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `kmeans2D` | method | Runs up to [maxIterations] of K-means; [points] are 2D, returns cluster index per point. |
...
| `knapsack01` | method | Solves the 0/1 knapsack: (max value, indices of chosen items). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `longestCommonSubsequence` | method | Returns one longest common subsequence of [a] and [b], compared with `==`. |
| `longestCommonSubsequenceLength` | method | Returns the length of the longest common subsequence of [a] and [b]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `longestCommonSubstringLength` | method | Returns the length of the longest common substring of [a] and [b]. |
| `longestCommonSubstring` | method | Returns one longest common substring of [a] and [b] (empty if none). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `multisetUnion` | method | Bag: count per element. |
| `multisetIntersection` | method | Returns the multiset intersection of [a] and [b], keeping each shared element with the minimum of its two counts. |
| `multisetDifference` | method | Returns the multiset difference [a] minus [b], subtracting counts and dropping any element whose remaining count falls to zero or below. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `pivot` | method | Pivot: one row per [rowKey] value, one column per [colKey] value, [valueKey] as cell. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `prefixFrequencyTable` | method | Builds map: prefix -> count of strings having that prefix. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `nthSmallest` | method | Returns the element that would sit at 0-based index [k] if [items] were sorted by [compare], without paying for a full sort. |
| `nthLargest` | method | Returns the [k]-th largest element (0-based: `k = 0` is the maximum), or `null` if [k] is out of range. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `reservoirSample` | method | Takes a sample of [k] items from [items] with equal probability per item (single pass). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `rollingHash` | method | Polynomial rolling hash of [s] in range [start, end). |
| `rollingHashSearch` | method | Returns index of [pattern] in [text] using rolling hash, or -1. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `columnarToRows` | method | From columnar map (key -> list of values) to list of row maps. |
...
| `runsEqual` | method | Detects runs of equal consecutive values in [list]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `bucketByTime` | method | Groups [events] (DateTime) into buckets of [bucketSize]; returns map of bucketStart -> count. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `topKIndices` | method | Returns indices of top [k] elements in [values] by [keyOf] (largest first). |
...
| `maxWeightIntervals` | method | Returns the maximum total weight of non-overlapping intervals (DP by end time). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `lag` | method | Lag: value at index [i - offset]; null if out of range. |
| `lead` | method | Lead: value at index [i + offset]; null if out of range. |
| `rowNumber` | method | Row number (1-based index). |
| `rank` | method | Rank: 1-based rank; equal values get same rank, next rank skips. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `businessDaysBetween` | method | Business days between two dates (excludes weekends; no holiday support). |
| `addBusinessDays` | method | Adds [n] business days to [date] (skips weekends). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Duration` | constructor | One full day, used in date boundary calculations. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Duration` | constructor | One day duration, used for day arithmetic. |
...
| `dateRange` | method | Generates a range of dates from [start] to [end] inclusive, stepping by [stepDays] days. |
...
| `durationBetween` | method | Returns the absolute [Duration] between [a] and [b], regardless of order. |
| `monthsBetween` | method | Returns the first day of each month from [start] through [end] inclusive. |
| `yearsBetween` | method | List of years from [start].year through [end].year (inclusive). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `dateRangeOverlap` | method | Overlap of two date ranges. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Duration` | constructor | One day duration used for month-end calculations. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `relativeTimeString` | method | Returns a relative time string: "2 hours ago", "in 3 days", "just now". |
...
| `parseIsoWeekString` | method | Parses ISO week string "2026-W09" to the Monday of that week. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `formatDuration` | method | Formats [Duration] as a short string (e.g. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseDuration` | method | Parses duration string like "1.5h", "90m", "2d 3h 30m". |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `splitByMonth` | method | Splits [start]..[end] into consecutive months. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `relativeDateBucket` | method | Bucket label for [date] relative to [today]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `roundMinutes` | method | Rounds [minutes] to nearest [step] (e.g. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `timebox` | method | Runs [fn]; if not completed in [timeout], returns [onTimeout] result (or throws). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `astar` | method | A* from [start] to [goal]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `bfs` | method | BFS from [start]; calls [visit](node, depth) for each node. |
| `dfs` | method | DFS from [start]; calls [visit](node, depth). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `bipartitePartition` | method | Returns (true, left, right) if bipartite, else (false, [], []). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `connectedComponents` | method | Returns list of components (each component = list of node indices). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `criticalPathDistances` | method | Longest path from [start] to each node (DAG). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `dagSchedule` | method | Returns schedule (list of node indices) respecting topology; [priority] lower = earlier. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `dijkstraDistances` | method | Returns shortest distances from [source] to all nodes; unreachable = infinity. |
| `dijkstraWithParents` | method | Returns (distances, parent array). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `floydWarshall` | method | Returns distance matrix where row i, column j gives shortest path i→j (infinity if none). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `graphEdgeDiff` | method | Compares two adjacency lists; returns (added edges, removed edges). |
...
| `buildWeightedGraph` | method | Builds [WeightedAdjacency] from [edges]; [nodeCount] = max node index + 1. |
| `Adjacency` | typedef | Unweighted: list of neighbors per node. |
| `buildGraph` | method | Builds an unweighted [Adjacency] list from [edges], where [nodeCount] is the max node index + 1. |
...
| `flattenHierarchy` | method | Flattens tree: [nodes] has (id, parentId). |
| `hierarchyFromParentIds` | method | Builds parent map from flat list: id -> parentId (roots have null). |
...
| `douglasPeuckerIndices` | method | Douglas–Peucker line simplification: returns indices of [points] to keep within [epsilon] tolerance. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `kruskalMST` | method | MST from [edges] (GraphUtils); returns (mst edges, total cost). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `topologicalSort` | method | Returns sorted node indices, or null if cycle detected. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `lowestCommonAncestor` | method | Parent array: entry at index i is the parent of node i; root has -1. |
| `treeDepths` | method | Returns depth of each node (root = 0). |
...
| `static` | method | Decodes a numeric entity at [offset] (`&#123;` or `&#xff;`). |
...
| `Function` | constructor | Sorts by [keyOf] and returns a new list. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Function` | constructor | Element with minimum [keyOf] value; null if empty. |
...
| `unzip2` | method | Splits [pairs] into two lists: first elements and second elements. |
...
| `Function` | constructor | Index of the element with minimum [keyOf]; null if empty. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Function` | constructor | Returns a new list sorted by [keyOf], keeping equal-key elements in their original relative order. |
...
| `runLengthDecode` | method | Run-length decode: expand (value, count) pairs into a list. |
...
| `mergeSorted` | method | Merge two sorted lists into one sorted list. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `deepEquals` | method | Deep equality: compares maps and lists recursively; primitives by value. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `deepFreeze` | method | Returns a deeply-unmodifiable copy of [value]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `deepCopyMap` | method | Deep copy for maps and lists. |
| `deepCopyList` | method | Recursively deep-copies [source], cloning any nested maps and lists so the result shares no mutable structure with the original. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `mapDiff` | method | Map diff: keys added, removed, changed. |
...
| `mapFromEntries` | method | Build map from iterable of (key, value) pairs. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `getNested` | method | Get/set nested value by path (list of keys). |
| `setNested` | method | Sets [value] at the nested location addressed by [path] in [map], creating intermediate maps as needed. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `additiveChecksum` | method | Simple additive checksum for integrity. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `hexToRgb` | method | Color hex to RGB. |
| `rgbToHex` | method | Packs [r], [g], and [b] channels into a fully opaque `0xAARRGGBB` color int. |
| `luminance` | method | Returns the relative luminance of an sRGB color per the WCAG 2.x formula. |
| `contrastRatio` | method | Returns the WCAG contrast ratio between two sRGB colors. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `simpleHash` | method | Simple hash for cache key from objects. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `abbreviateName` | method | Abbreviate name, initials from name. |
| `initialsFromName` | method | Returns the uppercase initials for [name]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `naturalCompare` | method | Sort strings naturally (human: a2 before a10). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `hexDump` | method | Renders [bytes] as a classic hex dump: offset, hex columns, and ASCII gutter. |
| `parseHexToBytes` | method | Parses a [hex] string into its byte values. |
| `bytesToHex` | method | Returns the lowercase, zero-padded hex string for [bytes], with no separators. |
| `maskCreditCard` | method | Masks all but the last [visibleLast] digits of [digits] with [maskChar]. |
| `stripControlChars` | method | Removes ASCII control characters (`0x00`–`0x1F` and `0x7F`) from [s]. |
| `isAsciiOnly` | method | Returns `true` if every code unit in [s] is within the ASCII range (`< 128`). |
| `truncateToByteLength` | method | Truncates [s] so its UTF-8 encoding does not exceed [maxBytes] bytes. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `padWithZeros` | method | Returns [value] as a string left-padded with zeros to at least [length] chars. |
| `formatFileSize` | method | Formats [bytes] as a human-readable size using binary (1024) units up to TB. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Random` | constructor | Generate random string (alphanumeric, length). |
| `randomAlphanumeric` | method | Returns a random alphanumeric string of [length] characters. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `stringDiffLines` | method | Diff two strings (line-by-line). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `gcd` | method | Greatest common divisor of [a] and [b]. |
| `lcm` | method | Least common multiple. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseCompactNumber` | method | Parse compact number ("1.2K" → 1200). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `factorial` | method | Factorial with overflow guard. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `lerp` | method | Lerp, inverse lerp, map value from one range to another. |
| `inverseLerp` | method | Returns where [value] falls between [a] and [b] as a fraction in `0.0`–`1.0`. |
| `mapRange` | method | Remaps [value] from the input range `[fromMin, fromMax]` to the output range `[toMin, toMax]`. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseNumberLocale` | method | Parses a number string with optional thousands separator. |
| `formatNumberLocale` | method | Formats number with [decimalPlaces] and optional [groupSep] (e.g. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `minOf` | method | Min/max of two or N numbers. |
| `maxOf` | method | Max of two numbers (NumUtils has maxOf with null handling). |
| `minOfMany` | method | Min of iterable; null if empty. |
| `maxOfMany` | method | Max of iterable; null if empty. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `modulo` | method | Modulo that handles negative (e.g. |
...
| `degreesToRadians` | method | Converts an angle in degrees ([deg]) to radians. |
| `radiansToDegrees` | method | Converts an angle in radians ([rad]) to degrees. |
| `normalizeAngle360` | method | Normalizes [degrees] into the range `[0, 360)`. |
| `normalizeAngle180` | method | Normalizes [degrees] into the range `(-180, 180]`. |
| `digitSum` | method | Returns the sum of the decimal digits of [n]. |
| `isPowerOfTwo` | method | Returns `true` if [n] is a positive power of two (`1, 2, 4, 8, …`). |
| `nextPowerOfTwo` | method | Returns the smallest power of two greater than or equal to [n]. |
| `isqrt` | method | Returns the integer square root of [n] (the floor of its real square root). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `isPrime` | method | Is prime (small numbers), prime factors. |
| `primeFactors` | method | Prime factors (small numbers). |
...
| `safeDivide` | method | Divides [a] by [b]; returns null if [b] is 0. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `variance` | method | Variance, standard deviation, median, percentile. |
| `standardDeviation` | method | Standard deviation (square root of variance). |
| `median` | method | Returns the median of [values], or `null` if the collection is empty. |
| `percentile` | method | Returns the [p]th percentile of [values] using linear interpolation. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `assertThat` | method | Assert with message (only in debug if desired). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `castOrNull` | method | Cast or null (safe cast). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `coalesce` | method | Null coalesce chain (first non-null of list). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `copyWithDefaults` | method | Copy with defaults (merge with default object). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `identityEquals` | method | Identity equality (same reference). |
...
| `isType` | method | Returns whether [value] is an instance of [T] at runtime. |
| `asTypeOr` | method | Returns [value] as a [T] when it is one, otherwise [fallback]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `Function` | constructor | Builds a function that threads its input left-to-right through [fns], passing each result to the next. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `also` | method | Runs [fn] for its side effects on [value] and returns [value] unchanged. |
| `let` | method | Applies [fn] to [value] and returns its result. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `requireNonNull` | method | Require non-null (throw descriptive if null). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `shallowCopyList` | method | Returns a new list with the same elements as [source]. |
| `shallowCopyMap` | method | Returns a new map with the same entries as [source]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `canonicalizeJson` | method | Recursively sorts map keys and normalizes numbers. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseChangelogSections` | method | Extracts sections like "## [1.0.0] - date" and content until next ##. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `mergeConfig` | method | Merges [defaults] with [overlay]; overlay wins for non-null values. |
...
| `detectCsvDialect` | method | Detects the dialect of a CSV/TSV [sample] by inspecting its first line. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `RegExp` | constructor | Email validation (reasonable regex, not RFC-perfect). |
| `isValidEmail` | method | Returns `true` if [email] looks like a valid address. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `luhnCheck` | method | Luhn check (credit card / ID validation). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseNestedQuery` | method | Parses [queryString] into a nested map. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseBool` | method | Parse simple boolean ("on"/"off", "1"/"0", "yes"/"no"). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseListFromString` | method | Parse list from string (e.g. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseIntBase` | method | Parsing More: parse int with base, validate URL/IP, parse port, validate hex, etc. |
| `isValidUrlLoose` | method | Returns `true` if [s] begins with an `http://` or `https://` scheme. |
| `isValidIpv4` | method | Returns `true` if [s] is a dotted-quad IPv4 address. |
| `parsePortFromHostPort` | method | Extracts the port number from a `host:port` string in [hostPort]. |
| `parseKeyValueLines` | method | Parses `key=value` lines from [input] into a map. |
| `isValidHexString` | method | Returns `true` if [s] consists only of hexadecimal digits. |
| `parseDottedDecimal` | method | Parses the dot-separated integers in [s] into a list. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `normalizePhoneDigits` | method | Phone number normalize (digits only or E.164-ish). |
| `normalizePhoneE164` | method | E.164-ish: digits only, optional leading + preserved as prefix for result. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `RegExp` | constructor | Parse size string ("1.5 MB", "512K") to bytes; format bytes to human. |
| `parseSizeToBytes` | method | Parses a human-readable size string in [input] into a byte count. |
| `formatBytesToHuman` | method | Formats a byte count in [bytes] into a human-readable size string. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `isNonEmptyAfterTrim` | method | Validate non-empty after trim. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `decodeVarint` | method | Decodes one varint from [bytes] at [start]. |
| `encodeVarint` | method | Encodes [value] as varint bytes. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `compareVersions` | method | Version compare (semver or dotted). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseVersion` | method | Parse version string (major.minor.patch). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `matchAll` | method | Match all (return all matches with groups). |
| `replaceAllWithCallback` | method | Replaces every match of [pattern] in [input] with the result of [replace]. |
| `namedGroupMap` | method | Returns the named capture groups of [match] as a map of name to value. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `confidenceInterval95` | method | Approximate 95% CI for mean: (lower, upper). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `pearsonCorrelation` | method | Pearson correlation between [x] and [y]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `zScoreNormalize` | method | Z-score: (x - mean) / stdDev. |
| `minMaxScale` | method | Min-max scale to [low, high]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `oneHot` | method | One-hot: [value] in [categories] -> list of 0/1 of length [categories]. |
| `bucketize` | method | Bucketize numeric value into bin index for [edges] (sorted boundaries). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `linearRegression` | method | Simple linear regression y ~ x. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `log1pSafe` | method | Log transform: log(1 + x) to handle zeros. |
| `expSafe` | method | Exp transform. |
| `logScale` | method | Log-scale list (1 + x then log). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `rollupDailyToPeriod` | method | Aggregates [dailyValues] (list of 7 or 30 values) into one value by [op]. |
| `rollupSum` | method | Sum of list. |
| `rollupAvg` | method | Average of list. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `simpleMovingAverage` | method | Simple moving average of [values] over window [size]. |
| `exponentialMovingAverage` | method | Exponential moving average: [alpha] in (0,1]; first value = first input. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `outlierIndicesByMAD` | method | Flags indices where \|x - median\| / MAD > [threshold]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `percentileRank` | method | Percentile rank of [value] in [sorted] (0..1). |
| `percentile` | method | Value at percentile [p] (0..1) in [sorted]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `retentionByDay` | method | [events] = list of (userId, date). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `median` | method | Median of [values] (assumes non-empty; sorts a copy). |
| `medianAbsoluteDeviation` | method | Median absolute deviation: median of \|x_i - median(x)\|. |
| `trimmedMean` | method | Trimmed mean: drop [trim] fraction from each tail (0..0.5), then average. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `systematicSample` | method | Systematic sampling: take every [step]-th element starting at [start]. |
| `stratifiedSampleIndices` | method | Stratified: [strata] gives group id per index; sample [perGroup] from each. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `extractAcronyms` | method | Extracts patterns like "Full Name (ABC)" -> (ABC, Full Name). |
...
| `applyPatch` | method | Applies an edit script [ops] to [baseText] (line-based). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `extractFirstCodeBlock` | method | Extracts first fenced code block ( |
| `extractAllCodeBlocks` | method | Returns all fenced code blocks as list of (language, code). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `didYouMean` | method | Returns up to [limit] words from [dictionary] closest to [word] by edit distance. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `isNearDuplicate` | method | Returns true if [a] and [b] are near-duplicates (cosine similarity >= [threshold]). |
| `clusterNearDuplicates` | method | Groups [documents] into near-duplicate clusters (greedy). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `stripEmailReplyQuotes` | method | Strips common reply patterns: lines starting with >, On ... |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `excerptAround` | method | Returns an excerpt of [text] around the first occurrence of [query], with [contextChars] characters on each side and [ellipsis] at truncation points. |
...
| `fuzzySearch` | method | Searches [candidates] for [query]; returns matches sorted by match score descending. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `sanitizeHtml` | method | Removes script and style tag contents, then strips all HTML tags. |
| `stripHtmlTags` | method | Strips all HTML tags and returns plain text (no attributes or script content). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `markdownToPlainText` | method | Strips Markdown markup and returns plain text (headers, lists as lines). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `extractSectionByHeading` | method | Extracts text under first heading matching [headingPattern] (e.g. |
| `extractFirstCodeBlock` | method | Returns first fenced code block content or null. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `characterNgrams` | method | Returns character n-grams of [s] with length [n]. |
| `wordNgrams` | method | Returns word n-grams of [s] with [n] words per gram. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `safeHtmlExcerpt` | method | Truncates [html] to about [maxLength] chars, closing open tags. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `deduplicateSlug` | method | Returns a slug that is not in [takenSlugs]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `lookupWithVariants` | method | Lookup that returns canonical key if [query] matches any variant within [maxDistance]. |
...
| `toUpperLatinOnly` | method | Example: |
...
| `RegExp` | constructor | Regular expression to remove punctuation while preserving alphabetic characters, **numbers**, and whitespace. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `substituteTemplate` | method | Replaces {{key}} in [template] with values from [data]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `textFingerprint` | method | Simple 32-bit fingerprint: hash of word shingles. |
| `fingerprintDistance` | method | Hamming distance between two 32-bit fingerprints (number of differing bits). |
...
| `normalizeText` | method | Applies [steps] in order to [text]. |
| `normalizeLower` | method | Predefined: lowercase. |
| `normalizeTrim` | method | Predefined: trim. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `termFrequencies` | method | Term frequencies for a list of tokens (e.g. |
| `cosineSimilarity` | method | Cosine similarity between two term-frequency maps (0.0 to 1.0). |
| `textToTf` | method | Tokenizes [s] by splitting on non-letters and lowercasing; returns TF map. |
| `textSimilarity` | method | Returns cosine similarity of [a] and [b] when treated as bags of words. |
...
| `extractUrlsWithContext` | method | Extracts URLs from [text]; [snippetLength] chars of context before/after. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `prettyPrint` | method | Recursively renders [obj] as an indented, human-readable string for debugging. |
| `dumpIterable` | method | Renders [it] as a string, truncating to the first [maxItems] elements and appending the total count when the iterable is longer. |
| `assertEqualsWithTolerance` | method | Throws an [AssertionError] when [a] and [b] differ by more than [tolerance]. |
| `rangeInt` | method | Returns integers from [start] (inclusive) toward [end] (exclusive) in increments of [step]. |
| `rangeDouble` | method | Returns doubles from [start] (inclusive) toward [end] (exclusive) in increments of [step]. |
| `repeatValue` | method | Returns a list containing [value] repeated [n] times. |
| `timed` | method | Runs [fn] synchronously and returns the wall-clock [Duration] it took. |
| `retryUntil` | method | Calls [predicate] up to [maxAttempts] times, returning `true` as soon as it succeeds, or `false` if every attempt fails. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `pathExtension` | method | File extension / without extension / change extension. |
| `pathWithoutExtension` | method | Returns [path] with its trailing file extension (and the dot) removed. |
| `pathChangeExtension` | method | Returns [path] with its extension replaced by [newExt]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `pathJoin` | method | Join path segments (cross-platform, no double slash). |
| `pathNormalize` | method | Normalize path (resolve . |
| `pathRelative` | method | Relative path from base to target. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `pathDirectory` | method | URL/Path More: directory from path, base name, path separator, is absolute, etc. |
| `pathBaseName` | method | Returns the final segment of [path] (the file or last directory name). |
| `pathSeparator` | getter | The forward-slash separator used by these path helpers. |
| `isPathAbsolute` | method | Whether [path] is absolute — starts with `/` or has a drive letter (`C:`). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `isAbsoluteUrl` | method | Is absolute URL / is relative path. |
| `isRelativePath` | method | Whether [path] is a relative path (no leading `/` and no drive letter). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `buildUri` | method | Build URL from base + path + query. |
| `stripFragment` | method | Returns [uri] with its fragment (`#...`) removed. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `canonicalizeUrl` | method | Returns a canonical form of [uri]: - scheme and host lower-cased (they are case-insensitive), - the scheme's default port removed — Dart's [Uri] does this au... |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `urlEncodeComponent` | method | Percent-encodes [value] for use as a single URI component. |
| `urlDecodeComponent` | method | Decodes a percent-encoded URI component [value] back to plain text. |
| `safeDecodeUri` | method | Decodes a percent-encoded URI component [value], returning null on failure. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseQueryString` | method | Parse query string to map (and back). |
| `buildQueryString` | method | Builds a percent-encoded `key=value&...` query string from [params]. |
| `uriWithQueryParams` | method | Add/remove query params (immutable). |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `generateUuidV4` | method | Generate random UUID v4. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `validateStartBeforeEnd` | method | Returns null if valid, else error message. |
| `validateOneOfRequired` | method | Returns null if at least one of [values] is non-null and non-empty (for strings). |
...
| `redactFields` | method | Redacts [data] by applying [mask] to values at keys in [fieldPaths]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `guardArgument` | method | Throws [ArgumentError] if [isConditionMet] is false, with [message]. |
| `guard` | method | Returns [value] if [isConditionMet] else throws [ArgumentError]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `clampNumber` | method | Clamps [value] to [min, max], then returns as int if [isInt] else double. |
| `shapeString` | method | Trims [s] and truncates to [maxLength] with optional [ellipsis]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `parseIpv4` | method | Parses IPv4 "a.b.c.d" into int (big-endian). |
| `ipInCidr` | method | Checks if [ip] (as int) is in CIDR [network]/[prefixLen]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `isJwtStructure` | method | Returns true if [token] looks like a JWT (three base64url parts separated by .). |
| `jwtPayload` | method | Decodes payload (middle part) as JSON map; returns null if invalid. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `passwordStrengthScore` | method | Simple strength score 0..4 from length and character variety. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `isPathSafe` | method | Returns true if [path] does not contain '..' segments that escape [root]. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `detectPii` | method | Detects likely PII patterns; returns list of (patternName, startIndex, endIndex). |
...
| `safeParse` | method | Parses [source] with [parse]; returns ParseOk or ParseErr. |
...
| Symbol | Kind | Description |
|--------|------|-------------|
| `safeTempName` | method | Returns a short random string suitable for temp file names (alphanumeric). |
