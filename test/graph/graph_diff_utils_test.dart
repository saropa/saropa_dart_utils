...
void main() {
  group('graphEdgeDiff', () {
    test('detects added and removed edges', () {
      ...
      final (List<(int, int)> added, List<(int, int)> removed) result =
          graphEdgeDiff(before, after);
    ...
    test('identical graphs produce no diff', () {
      ...
      final (List<(int, int)> added, List<(int, int)> removed) result =
          graphEdgeDiff(g, buildGraph(<(int, int)>[(0, 1), (1, 2)], 3));
    ...
    test('two empty graphs produce no diff', () {
      final (List<(int, int)> added, List<(int, int)> removed) result =
          graphEdgeDiff(<List<int>>[], <List<int>>[]);
      expect(result.$1, <(int, int)>[]);
      expect(result.$2, <(int, int)>[]);
    });
