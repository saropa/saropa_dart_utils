  ...
  group('TokenBucketRateLimiter', () {
    ...
    test('should allow an initial burst up to capacity', () {
      final TokenBucketRateLimiter limiter =
          TokenBucketRateLimiter(tokensPerSecond: 1, capacity: 3, now: clock);
    ...
    test('should refill at the configured rate as the clock advances', () {
      final TokenBucketRateLimiter limiter =
          TokenBucketRateLimiter(tokensPerSecond: 2, capacity: 2, now: clock);
    ...
    test('should cap refill at capacity (no overflow)', () {
      final TokenBucketRateLimiter limiter =
          TokenBucketRateLimiter(tokensPerSecond: 5, capacity: 3, now: clock);
    ...
    test('should not partially spend when tokens are insufficient', () {
      final TokenBucketRateLimiter limiter =
          TokenBucketRateLimiter(tokensPerSecond: 1, capacity: 5, now: clock);
    ...
    group('timeUntilAvailable', () {
      test('should be zero when tokens are available', () {
        final TokenBucketRateLimiter limiter =
            TokenBucketRateLimiter(tokensPerSecond: 1, capacity: 2, now: clock);
      ...
      test('should report the wait for the token deficit', () {
        final TokenBucketRateLimiter limiter =
            TokenBucketRateLimiter(tokensPerSecond: 2, capacity: 4, now: clock);
