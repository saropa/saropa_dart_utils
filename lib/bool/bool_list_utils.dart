extension BoolListExtensions on Iterable<bool> {
  ...
  ///
  /// This getter iterates through each element in the iterable. If it encounters an element that is `true`, it returns `true`. If no such element is found after iterating through all elements, it returns `false`.
  ///
  /// Returns:
  /// - `true` if at least one element in the iterable is `true`.
  /// - `false` if no elements in the iterable are `true` or the iterable is empty.
  bool get anyTrue {
  ...
  ///
  /// This getter iterates through each element in the iterable and increments a counter each time it encounters a `true` value. The final count is returned.
  ///
  /// Returns:
  ...
  static List<bool> reverse(List<bool> list) {
    return list.map((b) => !b).toList();
  }
}
