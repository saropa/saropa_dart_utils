...
class WeightedInterval {
  const WeightedInterval(int start, int end, int weight)
    : _start = start,
      _end = end,
      _weight = weight;
  final int _start;
  ...
  @override
  String toString() => 'WeightedInterval(start: $_start, end: $_end, weight: $_weight)';
}

/// Returns the maximum total weight of non-overlapping intervals (DP by end time).
int maxWeightIntervals(List<WeightedInterval> intervals) {
  if (intervals.isEmpty) return 0;
  final List<WeightedInterval> sorted = List<WeightedInterval>.of(intervals)
    ..sort((a, b) => a.end.compareTo(b.end));
  final List<int> dp = List.filled(sorted.length + 1, 0);
  for (int i = 1; i <= sorted.length; i++) {
    final WeightedInterval cur = sorted[i - 1];
    int j = i - 1;
