...
Fixes to the capabilities catalog generator so every utility is listed under its real name with a complete description.
[log](https://github.com/saropa/saropa_dart_utils/blob/main/CHANGELOG.md)

### Fixed

- **`CAPABILITIES.md` symbol names** ([tool/gen_capabilities.py](tool/gen_capabilities.py)) — the generator named each row after the first `Name(` on the declaration line, so methods whose return type, field initializer, or parameter type held a `Name(` were listed as `Function` / `RegExp` / `Duration` / `static`, and private members leaked when that token looked public. Names are now resolved from the declaration's own grammar: the identifier before the parameter list (the top-level `(` whose `)` is followed by the body), with balanced generic handling for nested bounds like `sortBy<K extends Comparable<K>>`, and the private filter applied to the source identifier. Function-typed public fields (e.g. `onFlush`) now keep their real name.
- **`CAPABILITIES.md` descriptions** ([tool/gen_capabilities.py](tool/gen_capabilities.py)) — first-sentence extraction cut at the first `. `, truncating descriptions at inline periods (`(e.g. 0.01 for 1%)`, `(split on . ! ?)`). It now ignores periods inside backticks, balanced parentheses, abbreviations (`e.g.`, `i.e.`, `vs.`), and single-letter dots, keeping the example intact.
- **`CAPABILITIES.md` internal markers** ([tool/gen_capabilities.py](tool/gen_capabilities.py)) — internal `— roadmap #NNN` planning markers are stripped from descriptions, and a file's "purpose" line is taken only from a genuine library-level doc, never promoted from the first member's dartdoc.
